        ...
        .filter(
          (n) =>
            (n.nutrient?.name === "Energy" &&
              (n.nutrient?.unitName === "kcal" ||
                n.nutrient?.unitName === "Cal")) ||
            [
              "Protein",
              "Total lipid (fat)",
              "Carbohydrate, by difference",
            ].includes(n.nutrient?.name),
        )
        .forEach((nutrient) => {
              ...
              <ul>
                {Object.entries(totalNutrients).map(([name, amount]) => (
                  <li key={name}>
                    ...
                    {nutrientsData[
                      recipe.ingredients[0]?.fdcid
                    ]?.foodNutrients.find((n) => n.nutrient?.name === name)
                      ?.nutrient?.unitName || ""}
                  </li>
                ))}
              </ul>
              <p className="text-muted">
                Note: This is an approximation of the nutritional value and may
                be incorrect.
              </p>
            </>
          )}
